...
def read_node_file(fopen, benchmark): #wtf why can you ignore all non-macro whywhywhywhywhywhywhywhywhwy?????
    ...
    port_info_raw_id_name ={}
    node_cnt = 0
    port_cnt = 0
    for line in fopen.readlines():
        if line[0:4] == "Cell":
            line = line.strip().split()
            cell_height = int(line[2])
            continue
        if not (line.startswith("\t") or line.startswith(" ")):
            continue
        line = line.strip().split()
        if int(line[2]) == cell_height:
            continue
        if line[-1] != "terminal":
            node_name = line[0]
            x = int(line[1])
            ...
            x = int(line[1])
            y = int(line[2])
            port_info[port_name] = {"id": port_cnt, "x": x, "y": y}
            port_info_raw_id_name[port_cnt] = port_name
            port_cnt += 1
    
    if cell_height == -1:
        print('not read in cell height information!! break....')
        sys.exit()

    print("len node_info", len(node_info))
    print("len port_info", len(port_info))
    return node_info, node_info_raw_id_name, port_info, port_info_raw_id_name, cell_height
    ...
    for line in fopen.readlines():
        ...
        if line[0] == "NetDegree":
            ...
            if node_name in node_info or node_name in port_info:
                ...
                if not node_name in net_info[net_name]["nodes"] and node_name[0] =='a': #node
                ...
                if not node_name in net_info[net_name]["ports"] and node_name[0] =='p': #port
                    net_info[net_name]["ports"][node_name] = {}
                    net_info[net_name]["ports"][node_name] = {"x_offset": 0, "y_offset": 0}
...
def read_pl_file(fopen, node_info, port_info):
    ...
    max_width = 0
    for line in fopen.readlines():
        if not line.startswith('a') and not line.startswith('p'):
            continue
        line = line.strip().split()
        node_name = line[0]
        ...
        if node_name[0] == 'a':
            ...
            max_height = max(max_height,   place_x)
            max_width = max(max_width,  place_y)
...
def read_scl_file(fopen):
    ...
    for line in fopen.readlines():
        if not (line.startswith("\t") or line.startswith(" ") or line.startswith("N")):
            continue
        line = line.strip().split()
        if line[0] == "NumRows" or line[0] == "Numrows":
            numrows = int(line[2])
        elif line[0] == "Height":
            if height_per_row == 0:
...
class PlaceDB_ibm():
    ...
    def __init__(self, benchmark = "adaptec1"):
        ...
        if benchmark == "ariane" or benchmark == "sample_clustered":
            ...
            node_file = open(os.path.join(benchmark+'_raw'+".nodes"), "r") 
            self.node_info, self.node_info_raw_id_name, self.port_info, self.port_to_net_dict, self.row_height = read_node_file(node_file, benchmark)  #read_node_file
            pl_file = open(os.path.join(benchmark+".pl"), "r")
            self.node_cnt = len(self.node_info)
            self.port_cnt = len(self.port_info)
            node_file.close()
            net_file = open(os.path.join(benchmark+".nets"), "r")
            self.net_info = read_net_file(net_file, self.node_info, self.port_info) #read_net_file
            self.net_cnt = len(self.net_info)
            net_file.close()
            pl_file = open(os.path.join(benchmark+".pl"), "r")
            self.max_height, self.max_width = read_pl_file(pl_file, self.node_info, self.port_info) #read_pl_file
            pl_file.close()
